...
export async function GET(request: NextRequest, context: { params: Promise<{ id: string }> }) {
  try {
    ...
    const member = await membersService.getById(id, access.context.congregationId);
...
export async function PUT(request: NextRequest, context: { params: Promise<{ id: string }> }) {
  try {
    ...
    const { id } = await context.params;
    const beforeData = await membersService.getById(id, access.context.congregationId);
    const body = memberUpdateSchema.parse(await request.json());
    const updatedMember = await membersService.update(id, body, access.context);
...
export async function DELETE(request: NextRequest, context: { params: Promise<{ id: string }> }) {
  try {
    ...
    const { id } = await context.params;
    const beforeData = await membersService.getById(id, access.context.congregationId);
    await membersService.remove(id, access.context);
